...
//        $userData =array('mobile'=>'13220113068',"userId"=>'201614747844623579');
     ...
     *
     */
    protected function getUserInfoByToken($token = 0){
        if(!$token) $this->_errorData(7788, '');
        $userData = $this->_checkToken($token);
        if($userData['userId']){
            $user = User1::find()->where(['user_id'=>$userData['userId']])->asArray()->one();
            if(!$user){
                $this->_errorData(7879, 'token错误');
            }
            return $user;
        }
        return array();
