...
//        $data = json_decode($this->get_php_file(__DIR__.'/jsapi_ticket.php'));
        $redis  = Yii::$app->cache;
        $update = Yii::$app->params['environment']."_jsapi_ticket";
        $data   = json_decode($redis->get($update));
        if(!$data){
            $data = (object)array();
            $data->expire_time = time() - 70000;
            $data->jsapi_ticket = '';
            $redis->set($update,json_encode($data));
        }
...
//        $data = json_decode($this->get_php_file(__DIR__.'/access_token.php'));
        $redis  = Yii::$app->cache;
        $update = Yii::$app->params['environment']."_access_token";
        $data   = json_decode($redis->get($update));
        if(!$data){
            $data = (object)array();
            $data->expire_time = time() - 70000;
