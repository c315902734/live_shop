...
class QcloudApi_Module_Vod extends QcloudApi_Module_Base
{
    ...
    public function MultipartUploadVodFile($params)
    {
        ...
        {
            $this->setError("", "API fileName参数为空，为必填选项\n");
            return false;
        }
        if(empty($params['fileName']))
        {
            $this->setError("", "fileName为空，请检测参数");
            return false;
        }
        if(!is_file($params['fileName']))
        {
            $this->setError("", $params['fileName']."  不存在，请检测\n");
            return false;
        }
        ...
        while(true)
        {
            ...
            if(!$fp)
            {
                $this->setError("", $params['fileName']." 文件打开错误");
                return false;
            }
            ...
            if(!$response)
            {
                ...
                {
                    $this->setError("", "retry ".$retry_times." times");
                    continue;
                }
            }
            ...
            if($response['code'] == 0)
            {
                ...
                if($response['flag'] == 1)
                {
                	$response['fileSha'] = $arguments['fileSha'];
                    return $response;
                }
